...
#	filebase = eval(input("请输入转换类型（1为到base64，2为到二进制，其余数字无效）："))
	input_suffix = filename[-3:]
	
#	if filebase == 1:
	if input_suffix == "txt":
		print("您当前正要执行[base64转二进制]，是否执行？")
		confirm = eval(input("（输入1执行，其他内容无效）："))
		if confirm == 1:
		...
		print("您当前正要执行[二进制转base64]，是否执行？")
		confirm = eval(input("（输入1执行，其他内容无效）："))
		if confirm == 1:
			ToBase64(f"./{filename}",f'{filename}_base64.txt')  # 文件转换为base64import base64
import time
...
#	if filebase == 1:
	if input_suffix == "txt": # 判断文件名最后三位是否为txt
		print("您当前正要执行[base64转二进制]，是否执行？")
		confirm = input("（输入1执行，其他内容无效）：")
		if confirm == '1':
			file_suffix_name = input("请输入转换后文件后缀名：")
			go = ToFile(f"./{filename}",f'{filename}_decode.{file_suffix_name}')  # base64编码转换为二进制文件
			time.sleep(5)
